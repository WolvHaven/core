...
data class MessagesConfig(
    ...
    data class TrainDestroy(
        ...
        val info: String =
            "<whprefix:TrainDestroy><whformat:info>Trains will be destroyed in <bold><destroy-in></bold></whformat><newline>" +
        ...
        @Comment(
            "Broadcast after /traindestroy delay is successfully ran\n" +
        ...
        @Comment(
            "Broadcast a few minutes prior to traindestroy running\n" +
                "<destroy-in>: time until destroy (MM:SS)",
        )
        val warning: String =
            "<whprefix:TrainDestroy><whformat:warning>Trains will be destroyed in <destroy-in></whformat><newline>" +
    ...
    data class CPolicing(
        ...
        val promptEnable: String =
            "<whprefix:CPolice><whformat:info>Community Policing can be enabled with " +
                "<u><click:run_command:/cpolicing enable>/cpolicing enable</click></u></whformat>",
        @Comment(
            "Broadcast when CPolicing changes state\n" +
        ...
        @Comment(
            "Response when attempting to change CPolice to the current state\n" +
                "<state>: either enabled or disabled",
        )
        val isAlreadyState: String = "<whprefix:CPolice><whformat:error>Community Policing is already <state>!</whformat>",
        @Comment(
            "Response when attempting to vote for an exempt player\n" +
        ...
        @Comment(
            "Response when attempting to vote for the same player multiple times\n" +
                "<target>: Username of the target player",
        )
        val alreadyVoted: String = "<whprefix:CPolice><whformat:error> You've already voted for <target>!</whformat>",
        @Comment(
            "Broadcast when someone votes successfully\n" +
        ...
        val vote: String =
            "<whprefix:CPolice><whformat:success> <source> has voted for <target>!</whformat> " +
                "<whformat:info>[<total-votes>/<threshold>]</whformat>",
        @Comment(
            "Broadcast when someone is successfully banned\n" +
                "<target>: Username of the target player",
        )
    ...
    data class InvisibleItemFrame(
        ...
        @Comment(
            "Response when an invisible item frame is given successfully\n" +
