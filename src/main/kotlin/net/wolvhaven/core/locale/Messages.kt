...
class Messages(private val plugin: CorePlugin) {
    val config =
        config<MessagesConfig>("messages", plugin).also {
            it.load()
            it.save()
        }
    ...
    private val resolver =
        TagResolver.builder()
            ...
            .tag("whformat") { args, _ ->
                val style =
                    when (args.popOr("no color!").value()) {
    ...
    inner class InvisibleItemFrames {
        fun itemName(isGlow: Boolean) =
            miniMessage.deserialize(if (isGlow) config().invisibleItemFrames.glowItemName else config().invisibleItemFrames.itemName)

        fun giveSuccess(isGlow: Boolean) =
            miniMessage.deserialize(
    ...
    inner class Core {
        ...
        fun reloadSuccess(module: String) =
            miniMessage.deserialize(
